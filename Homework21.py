...
o_validationSet_t = o_validationSet.transpose()
actual_y = o_validationSet_t[(len(o_validationSet_t)-1):][0]
arrayof1 = np.full((len(o_trainingSet), 1), 1)
t_set = o_trainingSet.transpose()
...
for i in range(0, len(o_validationSet)):
    predicted_y.append(thetas[0][0] + (thetas[1][0] * o_validationSet[i][6]) + (thetas[2][0] * o_validationSet[i][9]))
  
#calculate sum of square and mean square error for the data
sum_square_error = 0
for i in range(0, len(predicted_y)):
    sum_square_error = sum_square_error + (predicted_y[i] - actual_y[i])**2
